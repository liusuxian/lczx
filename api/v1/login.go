...
type LoginReq struct {
	...
	VerifyKey  string `json:"verifyKey" v:"required|regex:^[a-zA-Z0-9]*$#验证码key不能为空|验证码key格式错误"`                     // 验证码key
	VerifyCode string `json:"verifyCode" v:"required|max-length:4|regex:^[a-zA-Z0-9]*$#验证码不能为空|验证码不能超过4个字符|验证码格式错误"` // 验证码
}

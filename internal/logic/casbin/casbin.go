...
func init() {
	service.RegisterCasbin(newCasbin())
}

// 访问控制服务
func newCasbin() *sCasbin {
	return &sCasbin{}
}

// GetEnforcer 获取adapter单例对象
func (s *sCasbin) GetEnforcer(ctx context.Context) (enforcer *casbin.SyncedEnforcer, err error) {
	cCtx = ctx
	once.Do(func() {
		ac = s.newAdapter()
	})
...
func (a *adapterCasbin) SavePolicy(model model.Model) (err error) {
	...
	for pType, ast := range model["p"] {
		for _, rule := range ast.Policy {
			...
			_, err = dao.CasbinRule.Ctx(cCtx).Data(line).Insert()
			if err != nil {
				return
			}
		}
...
func (a *adapterCasbin) LoadPolicy(model model.Model) (err error) {
	...
	err = dao.CasbinRule.Ctx(cCtx).Scan(&lines)
	if err != nil {
		return
	}
...
func (a *adapterCasbin) initPolicy() {
	...
	e, err := casbin.NewSyncedEnforcer(
		g.Cfg().MustGet(cCtx, "casbin.modelFile").String(),
		g.Cfg().MustGet(cCtx, "casbin.policyFile").String(),
	)
	if err != nil {
		a.EnforcerErr = err
		return
...
func rawDelete(line *entity.CasbinRule) (err error) {
	dbModel := dao.CasbinRule.Ctx(cCtx).Where(do.CasbinRule{PType: line.PType})
	if line.V0 != "" {
